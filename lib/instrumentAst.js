...
function instrumentAst(ast, getNextLocationNumber) {
  function createRecordLocationExpression() {
    return {
      ...
      arguments: [{ type: 'Literal', value: getNextLocationNumber() }],
    };
  }
  ...
  ast = estraverse.replace(ast, {
    ...
    leave(node) {
      if (
        node.type === 'CallExpression' &&
        node.callee.type === 'Identifier' &&
        (node.callee.name === 'recordLocation' ||
          node.callee.name === 'recordProximity')
      ) {
        // Skip the injected instrumentation code so we don't gather magic values from it :)
        return this.skip();
      } else if (
        node.type === 'BinaryExpression' &&
        binaryOperatorsToWrap.has(node.operator)
      ) {
        return {
          type: 'CallExpression',
          callee: {
        ...
        if (node.body.type === 'BlockStatement') {
          ...
          node.body = {
            ...
            expressions: [createRecordLocationExpression(), node.body],
          };
        }
        ...
        node.consequent = {
          type: 'SequenceExpression',
          expressions: [createRecordLocationExpression(), node.consequent],
        };
        node.alternate = {
          type: 'SequenceExpression',
          expressions: [createRecordLocationExpression(), node.alternate],
        };
      } else if (
        ...
        node.right = {
          type: 'SequenceExpression',
          expressions: [createRecordLocationExpression(), node.right],
        };
      }
    },
  });
